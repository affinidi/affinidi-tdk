...
class FeatureDiscoveryHelper {
  ...
  static VdspFeatureSelection _selectFirstSupportedPerFeatureType(
    ...
    final vcTypeDisclosures = grouped[FeatureType.vcType.value];
    if (vcTypeDisclosures != null && vcTypeDisclosures.isNotEmpty) {
      final vcTypeId = vcTypeDisclosures.first.id;
      vcType = VcType.values
          .cast<VcType?>()
          .firstWhere((e) => e?.value == vcTypeId, orElse: () => null);
    }
...
class VdspFeatureSelection {
  ...
  @override
  String toString() {
    return 'VdspFeatureSelection(protocol: $protocol, dataQueryLanguage: ${dataQueryLanguage?.value}, vcType: ${vcType?.value}, dataIntegrityProofSuite: ${dataIntegrityProofSuite?.value}, jsonWebSignatureAlgorithm: ${jsonWebSignatureAlgorithm?.value}, operation: $operation)';
  }
}
