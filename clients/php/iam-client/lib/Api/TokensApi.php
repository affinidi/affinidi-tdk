...
class TokensApi
{
     ...
     * @throws \AffinidiTdk\Clients\Iam\ApiException on non-2xx response or if the response body is not in the expected format
     * @throws \InvalidArgumentException
     * @return \AffinidiTdk\Clients\Iam\Model\ProjectWithPolicyList|\AffinidiTdk\Clients\Iam\Model\ActionForbiddenError|\AffinidiTdk\Clients\Iam\Model\NotFoundError|\AffinidiTdk\Clients\Iam\Model\UnexpectedError
     */
     ...
     * @throws \AffinidiTdk\Clients\Iam\ApiException on non-2xx response or if the response body is not in the expected format
     * @throws \InvalidArgumentException
     * @return array of \AffinidiTdk\Clients\Iam\Model\ProjectWithPolicyList|\AffinidiTdk\Clients\Iam\Model\ActionForbiddenError|\AffinidiTdk\Clients\Iam\Model\NotFoundError|\AffinidiTdk\Clients\Iam\Model\UnexpectedError, HTTP status code, HTTP response headers (array of strings)
     */
    public function listProjectsOfTokenWithHttpInfo($token_id, $limit = null, $exclusive_start_key = null, string $contentType = self::contentTypes['listProjectsOfToken'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\AffinidiTdk\Clients\Iam\Model\ProjectWithPolicyList' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\AffinidiTdk\Clients\Iam\Model\ProjectWithPolicyList' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\AffinidiTdk\Clients\Iam\Model\ProjectWithPolicyList', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                case 403:
            ...
            $returnType = '\AffinidiTdk\Clients\Iam\Model\ProjectWithPolicyList';
            if ($returnType === '\SplFileObject') {
                $content = $response->getBody(); //stream goes to serializer
            } else {
                $content = (string) $response->getBody();
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\AffinidiTdk\Clients\Iam\Model\ProjectWithPolicyList',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
    ...
    public function listProjectsOfTokenRequest($token_id, $limit = null, $exclusive_start_key = null, string $contentType = self::contentTypes['listProjectsOfToken'][0])
    {
        ...
        if ($token_id === null || (is_array($token_id) && count($token_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $token_id when calling listProjectsOfToken'
            );
        }
