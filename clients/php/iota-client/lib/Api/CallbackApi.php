...
class CallbackApi
{
    ...
    {
        list($response) = $this->iotOIDC4VPCallbackWithHttpInfo($callback_input, $contentType);
        return $response;
    }
     ...
     */
    public function iotOIDC4VPCallbackAsync($callback_input, string $contentType = self::contentTypes['iotOIDC4VPCallback'][0])
    {
        return $this->iotOIDC4VPCallbackAsyncWithHttpInfo($callback_input, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     */
    public function iotOIDC4VPCallbackRequest($callback_input, string $contentType = self::contentTypes['iotOIDC4VPCallback'][0])
    {

        // verify the required parameter 'callback_input' is set
        if ($callback_input === null || (is_array($callback_input) && count($callback_input) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $callback_input when calling iotOIDC4VPCallback'
            );
        }


        $resourcePath = '/v1/callback';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        // for model (json/xml)
        if (isset($callback_input)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($callback_input));
            } else {
                $httpBody = $callback_input;
            }
