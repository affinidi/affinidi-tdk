# coding: utf-8

"""
    CredentialIssuanceService

    Affinidi Credential Issuance Service Structure

    The version of the OpenAPI document: 1.0.0
    Contact: nucleus.team@affinidi.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from affinidi_tdk_client_cis.api.credentials_api import CredentialsApi  # noqa: E501


class TestCredentialsApi(unittest.TestCase):
    """CredentialsApi unit test stubs"""

    def setUp(self) -> None:
        self.api = CredentialsApi()  # noqa: E501

    def tearDown(self) -> None:
        pass

    def test_deferred_credentials(self) -> None:
        """Test case for deferred_credentials

        """
        pass

    def test_generate_credentials(self) -> None:
        """Test case for generate_credentials

        """
        pass


if __name__ == '__main__':
    unittest.main()
