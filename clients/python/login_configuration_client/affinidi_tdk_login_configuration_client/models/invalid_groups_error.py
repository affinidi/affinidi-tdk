...
class InvalidGroupsError(BaseModel):
    """
    InvalidGroupsError
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('InvalidGroupsError'):
            raise ValueError("must be one of enum values ('InvalidGroupsError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Invalid groups names'):
            raise ValueError("must be one of enum values ('Invalid groups names')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> InvalidGroupsError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> InvalidGroupsError:
        """Create an instance of InvalidGroupsError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return InvalidGroupsError.parse_obj(obj)

        _obj = InvalidGroupsError.parse_obj({
            "name": obj.get("name"),
