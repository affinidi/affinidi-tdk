...
class VPTokenValidationError(BaseModel):
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('VPTokenValidationError'):
            raise ValueError("must be one of enum values ('VPTokenValidationError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('VP token validation ended with an error'):
            raise ValueError("must be one of enum values ('VP token validation ended with an error')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VPTokenValidationError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> VPTokenValidationError:
        """Create an instance of VPTokenValidationError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return VPTokenValidationError.parse_obj(obj)

        _obj = VPTokenValidationError.parse_obj({
            "name": obj.get("name"),
