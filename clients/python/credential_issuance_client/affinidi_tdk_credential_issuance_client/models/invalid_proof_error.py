...
class InvalidProofError(BaseModel):
    """
    InvalidProofError
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('InvalidProofError'):
            raise ValueError("must be one of enum values ('InvalidProofError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('The proof in the Credential Request is invalid'):
            raise ValueError("must be one of enum values ('The proof in the Credential Request is invalid')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> InvalidProofError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> InvalidProofError:
        """Create an instance of InvalidProofError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return InvalidProofError.parse_obj(obj)

        _obj = InvalidProofError.parse_obj({
            "name": obj.get("name"),
