...
class InvalidCredentialTypeError(BaseModel):
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('InvalidCredentialTypeError'):
            raise ValueError("must be one of enum values ('InvalidCredentialTypeError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('The requested credential type is not supported'):
            raise ValueError("must be one of enum values ('The requested credential type is not supported')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> InvalidCredentialTypeError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> InvalidCredentialTypeError:
        """Create an instance of InvalidCredentialTypeError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return InvalidCredentialTypeError.parse_obj(obj)

        _obj = InvalidCredentialTypeError.parse_obj({
            "name": obj.get("name"),
