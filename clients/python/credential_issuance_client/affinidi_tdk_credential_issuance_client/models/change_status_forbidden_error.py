...
class ChangeStatusForbiddenError(BaseModel):
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('ChangeStatusForbiddenError'):
            raise ValueError("must be one of enum values ('ChangeStatusForbiddenError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Related VC status cannot be changed'):
            raise ValueError("must be one of enum values ('Related VC status cannot be changed')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ChangeStatusForbiddenError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ChangeStatusForbiddenError:
        """Create an instance of ChangeStatusForbiddenError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ChangeStatusForbiddenError.parse_obj(obj)

        _obj = ChangeStatusForbiddenError.parse_obj({
            "name": obj.get("name"),
