...
class VcClaimedError(BaseModel):
    """
    VcClaimedError
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('VcClaimedError'):
            raise ValueError("must be one of enum values ('VcClaimedError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('The requested VC has already been claimed by the user'):
            raise ValueError("must be one of enum values ('The requested VC has already been claimed by the user')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VcClaimedError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> VcClaimedError:
        """Create an instance of VcClaimedError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return VcClaimedError.parse_obj(obj)

        _obj = VcClaimedError.parse_obj({
            "name": obj.get("name"),
