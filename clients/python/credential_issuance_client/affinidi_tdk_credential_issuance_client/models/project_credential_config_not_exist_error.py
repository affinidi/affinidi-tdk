...
class ProjectCredentialConfigNotExistError(BaseModel):
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('ProjectCredentialConfigNotExistError'):
            raise ValueError("must be one of enum values ('ProjectCredentialConfigNotExistError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Credential config is not available for the project'):
            raise ValueError("must be one of enum values ('Credential config is not available for the project')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ProjectCredentialConfigNotExistError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ProjectCredentialConfigNotExistError:
        """Create an instance of ProjectCredentialConfigNotExistError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ProjectCredentialConfigNotExistError.parse_obj(obj)

        _obj = ProjectCredentialConfigNotExistError.parse_obj({
            "name": obj.get("name"),
