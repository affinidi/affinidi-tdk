...
class TooManyRequestsError(BaseModel):
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('TooManyRequestsError',):
            raise ValueError("must be one of enum values ('TooManyRequestsError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Too many concurrent requests to the same wallet. The operation failed after multiple retry attempts due to concurrent updates. Please retry your request after a short delay.',):
            raise ValueError("must be one of enum values ('Too many concurrent requests to the same wallet. The operation failed after multiple retry attempts due to concurrent updates. Please retry your request after a short delay.')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> TooManyRequestsError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> TooManyRequestsError:
        """Create an instance of TooManyRequestsError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return TooManyRequestsError.parse_obj(obj)

        _obj = TooManyRequestsError.parse_obj({
            "name": obj.get("name"),
