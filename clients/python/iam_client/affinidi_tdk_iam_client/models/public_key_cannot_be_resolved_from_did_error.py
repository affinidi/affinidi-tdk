...
class PublicKeyCannotBeResolvedFromDidError(BaseModel):
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('PublicKeyCannotBeResolvedFromDidError'):
            raise ValueError("must be one of enum values ('PublicKeyCannotBeResolvedFromDidError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Unable to resolve DID method. Invalid public key'):
            raise ValueError("must be one of enum values ('Unable to resolve DID method. Invalid public key')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PublicKeyCannotBeResolvedFromDidError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> PublicKeyCannotBeResolvedFromDidError:
        """Create an instance of PublicKeyCannotBeResolvedFromDidError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PublicKeyCannotBeResolvedFromDidError.parse_obj(obj)

        _obj = PublicKeyCannotBeResolvedFromDidError.parse_obj({
            "name": obj.get("name"),
