...
| [**get_access_vfs**](AuthzApi.md#get_access_vfs)       | **GET** /v1/authz/vfs/access/{granteeDid}    | Get permissions to the virtual file system for a subject |
...
> GetAccessOutput get_access_vfs(grantee_did)
...
with affinidi_tdk_consumer_iam_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Get permissions to the virtual file system for a subject
        api_response = api_instance.get_access_vfs(grantee_did)
        print("The response of AuthzApi->get_access_vfs:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling AuthzApi->get_access_vfs: %s\n" % e)
```
