...
class MappingsResponse(BaseModel):
    """
    Mappings between common schemas and connectors  # noqa: E501
    """
    mappings: Dict[str, conlist(BasicConnectorDTO)] = Field(..., description="A dictionary containing mappings between a common schema and a list of connectors")
    __properties = ["mappings"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MappingsResponse:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each value in mappings (dict of array)
        _field_dict_of_array = {}
        if self.mappings:
            for _key in self.mappings:
                if self.mappings[_key]:
                    _field_dict_of_array[_key] = [
                        _item.to_dict() for _item in self.mappings[_key]
                    ]
            _dict['mappings'] = _field_dict_of_array
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> MappingsResponse:
        """Create an instance of MappingsResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MappingsResponse.parse_obj(obj)

        _obj = MappingsResponse.parse_obj({
            "mappings": dict(
                (_k,
                        [BasicConnectorDTO.from_dict(_item) for _item in _v]
                        if _v is not None
                        else None
                )
                for _k, _v in obj.get("mappings").items()
            )
        })
        return _obj
