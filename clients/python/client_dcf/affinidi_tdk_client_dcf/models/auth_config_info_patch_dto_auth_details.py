...
class AuthConfigInfoPatchDtoAuthDetails(BaseModel):
    """
    The authentication information needed to connect with a Data Source.   # noqa: E501
    """
    auth_url: Optional[constr(strict=True, max_length=1024, min_length=1)] = Field(None, alias="authUrl", description="The base URL with which the data connector framework (DCF) can get authenticated for using the connector. Should be obtained from the API documentation of the third-party service. ")
    template: Optional[StrictStr] = Field(None, description="The template for generating the full authentication URL for authenticating the DCF. ")
    __properties = ["authUrl", "template"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AuthConfigInfoPatchDtoAuthDetails:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AuthConfigInfoPatchDtoAuthDetails:
        """Create an instance of AuthConfigInfoPatchDtoAuthDetails from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AuthConfigInfoPatchDtoAuthDetails.parse_obj(obj)

        _obj = AuthConfigInfoPatchDtoAuthDetails.parse_obj({
