...
class DataSourceConfigResponse(BaseModel):
    """
    List of all available configurations of data sources.   # noqa: E501
    """
    data_source_configs: conlist(AuthConfig) = Field(..., alias="dataSourceConfigs", description="The list of all available data source configurations.")
    __properties = ["dataSourceConfigs"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DataSourceConfigResponse:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in data_source_configs (list)
        _items = []
        if self.data_source_configs:
            for _item in self.data_source_configs:
                if _item:
                    _items.append(_item.to_dict())
            _dict['dataSourceConfigs'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> DataSourceConfigResponse:
        """Create an instance of DataSourceConfigResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DataSourceConfigResponse.parse_obj(obj)

        _obj = DataSourceConfigResponse.parse_obj({
            "data_source_configs": [AuthConfig.from_dict(_item) for _item in obj.get("dataSourceConfigs")] if obj.get("dataSourceConfigs") is not None else None
        })
        return _obj
