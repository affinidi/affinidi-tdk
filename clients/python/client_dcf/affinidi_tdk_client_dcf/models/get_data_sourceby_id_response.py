...
GETDATASOURCEBYIDRESPONSE_ONE_OF_SCHEMAS = ["DataSource", "DataSourceValidationResult"]

class GetDataSourcebyIdResponse(BaseModel):
    """
    GetDataSourcebyIdResponse
    """
    # data type: DataSource
    oneof_schema_1_validator: Optional[DataSource] = None
    # data type: DataSourceValidationResult
    oneof_schema_2_validator: Optional[DataSourceValidationResult] = None
    if TYPE_CHECKING:
        actual_instance: Union[DataSource, DataSourceValidationResult]
    else:
        actual_instance: Any
    one_of_schemas: List[str] = Field(GETDATASOURCEBYIDRESPONSE_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = GetDataSourcebyIdResponse.construct()
        error_messages = []
        match = 0
        # validate data type: DataSource
        if not isinstance(v, DataSource):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DataSource`")
        else:
            match += 1
        # validate data type: DataSourceValidationResult
        if not isinstance(v, DataSourceValidationResult):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DataSourceValidationResult`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in GetDataSourcebyIdResponse with oneOf schemas: DataSource, DataSourceValidationResult. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in GetDataSourcebyIdResponse with oneOf schemas: DataSource, DataSourceValidationResult. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> GetDataSourcebyIdResponse:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> GetDataSourcebyIdResponse:
        """Returns the object represented by the json string"""
        instance = GetDataSourcebyIdResponse.construct()
        error_messages = []
        match = 0

        # deserialize data into DataSource
        try:
            instance.actual_instance = DataSource.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into DataSourceValidationResult
        try:
            instance.actual_instance = DataSourceValidationResult.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into GetDataSourcebyIdResponse with oneOf schemas: DataSource, DataSourceValidationResult. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetDataSourcebyIdResponse with oneOf schemas: DataSource, DataSourceValidationResult. Details: " + ", ".join(error_messages))
        else:
            return instance
