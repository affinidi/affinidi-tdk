...
class AuthConfigInfoPatchDtoRefreshTokenTemplate(BaseModel):
    """
    Template for defining the requirements of renewing the authentication token with the external data source.   # noqa: E501
    """
    options: Optional[Dict[str, Any]] = Field(None, description="Defines all request and auth headers which are common for each operation. ")
    operations: Optional[conlist(AuthConfigInfoPatchDtoRefreshTokenTemplateOperationsInner)] = Field(None, description="A list of REST operations to retrieve an exchange token from the data source. ")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AuthConfigInfoPatchDtoRefreshTokenTemplate:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AuthConfigInfoPatchDtoRefreshTokenTemplate:
        """Create an instance of AuthConfigInfoPatchDtoRefreshTokenTemplate from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AuthConfigInfoPatchDtoRefreshTokenTemplate.parse_obj(obj)

        _obj = AuthConfigInfoPatchDtoRefreshTokenTemplate.parse_obj({
            "options": obj.get("options"),
            "operations": [AuthConfigInfoPatchDtoRefreshTokenTemplateOperationsInner.from_dict(_item) for _item in obj.get("operations")] if obj.get("operations") is not None else None
        })
        return _obj
