...
class AuthConfigInfoPatchDto(BaseModel):
    """
    The information needed to onboard and use a data source.   # noqa: E501
    """
    auth_details: Optional[AuthConfigInfoPatchDtoAuthDetails] = Field(None, alias="authDetails")
    exchange_token_template: Optional[AuthConfigInfoPatchDtoExchangeTokenTemplate] = Field(None, alias="exchangeTokenTemplate")
    refresh_token_template: Optional[AuthConfigInfoPatchDtoRefreshTokenTemplate] = Field(None, alias="refreshTokenTemplate")
    __properties = ["authDetails", "exchangeTokenTemplate", "refreshTokenTemplate"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AuthConfigInfoPatchDto:
    ...
    def to_dict(self):
        ...
        if self.auth_details:
            _dict['authDetails'] = self.auth_details.to_dict()
        # override the default output from pydantic by calling `to_dict()` of exchange_token_template
        if self.exchange_token_template:
            _dict['exchangeTokenTemplate'] = self.exchange_token_template.to_dict()
        # override the default output from pydantic by calling `to_dict()` of refresh_token_template
        if self.refresh_token_template:
            _dict['refreshTokenTemplate'] = self.refresh_token_template.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> AuthConfigInfoPatchDto:
        """Create an instance of AuthConfigInfoPatchDto from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AuthConfigInfoPatchDto.parse_obj(obj)

        _obj = AuthConfigInfoPatchDto.parse_obj({
            "auth_details": AuthConfigInfoPatchDtoAuthDetails.from_dict(obj.get("authDetails")) if obj.get("authDetails") is not None else None,
            "exchange_token_template": AuthConfigInfoPatchDtoExchangeTokenTemplate.from_dict(obj.get("exchangeTokenTemplate")) if obj.get("exchangeTokenTemplate") is not None else None,
            "refresh_token_template": AuthConfigInfoPatchDtoRefreshTokenTemplate.from_dict(obj.get("refreshTokenTemplate")) if obj.get("refreshTokenTemplate") is not None else None
        })
        return _obj
