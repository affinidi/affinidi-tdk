...
class ExpiredCodeExceptionError(BaseModel):
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('ExpiredCodeExceptionError'):
            raise ValueError("must be one of enum values ('ExpiredCodeExceptionError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Code has expired'):
            raise ValueError("must be one of enum values ('Code has expired')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ExpiredCodeExceptionError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ExpiredCodeExceptionError:
        """Create an instance of ExpiredCodeExceptionError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ExpiredCodeExceptionError.parse_obj(obj)

        _obj = ExpiredCodeExceptionError.parse_obj({
            "name": obj.get("name"),
