...
class UsernameExistsExceptionError(BaseModel):
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('UsernameExistsExceptionError'):
            raise ValueError("must be one of enum values ('UsernameExistsExceptionError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Username already exists'):
            raise ValueError("must be one of enum values ('Username already exists')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> UsernameExistsExceptionError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> UsernameExistsExceptionError:
        """Create an instance of UsernameExistsExceptionError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return UsernameExistsExceptionError.parse_obj(obj)

        _obj = UsernameExistsExceptionError.parse_obj({
            "name": obj.get("name"),
