...
class CodeMismatchExceptionError(BaseModel):
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('CodeMismatchExceptionError'):
            raise ValueError("must be one of enum values ('CodeMismatchExceptionError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Invalid code'):
            raise ValueError("must be one of enum values ('Invalid code')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CodeMismatchExceptionError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CodeMismatchExceptionError:
        """Create an instance of CodeMismatchExceptionError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CodeMismatchExceptionError.parse_obj(obj)

        _obj = CodeMismatchExceptionError.parse_obj({
            "name": obj.get("name"),
