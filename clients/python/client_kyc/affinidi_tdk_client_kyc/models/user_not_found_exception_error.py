...
class UserNotFoundExceptionError(BaseModel):
    """
    ...
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('UserNotFoundExceptionError'):
            raise ValueError("must be one of enum values ('UserNotFoundExceptionError')")
        return value

    @validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('User cannot be found. Please enter email address again.'):
            raise ValueError("must be one of enum values ('User cannot be found. Please enter email address again.')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> UserNotFoundExceptionError:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> UserNotFoundExceptionError:
        """Create an instance of UserNotFoundExceptionError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return UserNotFoundExceptionError.parse_obj(obj)

        _obj = UserNotFoundExceptionError.parse_obj({
            "name": obj.get("name"),
