...
class AuthConfigInfoDtoAuthDetails(BaseModel):
    """
    The authentication information needed to connect with a data source.   # noqa: E501
    """
    auth_url: constr(strict=True, max_length=1024, min_length=1) = Field(..., alias="authUrl", description="The base URL with which the data connector framework (DCF) can get authenticated for using the connector. Should be obtained from the API documentation of the third-party service. ")
    template: StrictStr = Field(..., description="The template for generating the full authentication URL for authenticating the DCF. ")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AuthConfigInfoDtoAuthDetails:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AuthConfigInfoDtoAuthDetails:
        """Create an instance of AuthConfigInfoDtoAuthDetails from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AuthConfigInfoDtoAuthDetails.parse_obj(obj)

        _obj = AuthConfigInfoDtoAuthDetails.parse_obj({
