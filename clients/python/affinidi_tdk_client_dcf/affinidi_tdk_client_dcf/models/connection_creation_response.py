...
class ConnectionCreationResponse(BaseModel):
    """
    Response payload after successful creation of a connection.   # noqa: E501
    """
    sso_start_url: Optional[StrictStr] = Field(None, alias="ssoStartUrl", description="the Single-Sign-On address through which a user need to authenticate the connection  with the use of the data connector framework (DCF). ")
    __properties = ["ssoStartUrl"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ConnectionCreationResponse:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ConnectionCreationResponse:
        """Create an instance of ConnectionCreationResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ConnectionCreationResponse.parse_obj(obj)

        _obj = ConnectionCreationResponse.parse_obj({
            "sso_start_url": obj.get("ssoStartUrl")
        })
        return _obj
