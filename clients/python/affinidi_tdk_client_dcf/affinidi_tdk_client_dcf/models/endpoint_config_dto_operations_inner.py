...
class EndpointConfigDtoOperationsInner(BaseModel):
    """
    Describes the configuration for a single REST operation for the data connector.   # noqa: E501
    """
    ...
    functions: Dict[str, Any] = Field(..., description="An object defining functions which process the received data from the defined operation. ")
    pagination: Optional[Dict[str, Any]] = Field(None, description="An optional object defining how paginated results will be collected. ")
    source_schema: Dict[str, Any] = Field(..., alias="sourceSchema", description="Schema of the response returned by the chosen API endpoint. It serves as a reference to understand the structure and format of data returned by the data source API. ")
    __properties = ["template", "functions", "pagination", "sourceSchema"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> EndpointConfigDtoOperationsInner:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> EndpointConfigDtoOperationsInner:
        """Create an instance of EndpointConfigDtoOperationsInner from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return EndpointConfigDtoOperationsInner.parse_obj(obj)

        _obj = EndpointConfigDtoOperationsInner.parse_obj({
            "template": obj.get("template"),
            "functions": obj.get("functions"),
            "pagination": obj.get("pagination"),
            "source_schema": obj.get("sourceSchema")
        })
        return _obj
