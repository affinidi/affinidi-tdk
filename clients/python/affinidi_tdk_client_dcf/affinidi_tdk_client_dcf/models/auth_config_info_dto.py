...
class AuthConfigInfoDto(BaseModel):
    """
    The information needed to onboard and use a data source.   # noqa: E501
    """
    auth_details: AuthConfigInfoDtoAuthDetails = Field(..., alias="authDetails")
    exchange_token_template: AuthConfigInfoDtoExchangeTokenTemplate = Field(..., alias="exchangeTokenTemplate")
    refresh_token_template: Optional[AuthConfigInfoDtoRefreshTokenTemplate] = Field(None, alias="refreshTokenTemplate")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AuthConfigInfoDto:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AuthConfigInfoDto:
        """Create an instance of AuthConfigInfoDto from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AuthConfigInfoDto.parse_obj(obj)

        _obj = AuthConfigInfoDto.parse_obj({
            "auth_details": AuthConfigInfoDtoAuthDetails.from_dict(obj.get("authDetails")) if obj.get("authDetails") is not None else None,
            "exchange_token_template": AuthConfigInfoDtoExchangeTokenTemplate.from_dict(obj.get("exchangeTokenTemplate")) if obj.get("exchangeTokenTemplate") is not None else None,
            "refresh_token_template": AuthConfigInfoDtoRefreshTokenTemplate.from_dict(obj.get("refreshTokenTemplate")) if obj.get("refreshTokenTemplate") is not None else None
        })
        return _obj
