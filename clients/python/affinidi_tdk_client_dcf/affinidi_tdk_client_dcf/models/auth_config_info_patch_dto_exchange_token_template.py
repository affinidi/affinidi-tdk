...
class AuthConfigInfoPatchDtoExchangeTokenTemplate(BaseModel):
    """
    Template for defining the requirements of authentication token exchange with the external data source.   # noqa: E501
    """
    options: Optional[Dict[str, Any]] = Field(None, description="Defines all request and auth headers which are common for each operation.s ")
    operations: Optional[conlist(AuthConfigInfoPatchDtoExchangeTokenTemplateOperationsInner)] = Field(None, description="A list of REST operations to exchange authentication token with the data source. ")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AuthConfigInfoPatchDtoExchangeTokenTemplate:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AuthConfigInfoPatchDtoExchangeTokenTemplate:
        """Create an instance of AuthConfigInfoPatchDtoExchangeTokenTemplate from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AuthConfigInfoPatchDtoExchangeTokenTemplate.parse_obj(obj)

        _obj = AuthConfigInfoPatchDtoExchangeTokenTemplate.parse_obj({
            "options": obj.get("options"),
            "operations": [AuthConfigInfoPatchDtoExchangeTokenTemplateOperationsInner.from_dict(_item) for _item in obj.get("operations")] if obj.get("operations") is not None else None
        })
        return _obj
