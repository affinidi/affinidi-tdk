...
class PatchDataSourceConfigStatusResponse(BaseModel):
    """
    The response for activating or deactivating   # noqa: E501
    """
    sso_url: Optional[StrictStr] = Field(None, alias="ssoUrl")
    data_source_config: Optional[DataSourceConfig] = Field(None, alias="dataSourceConfig")
    __properties = ["ssoUrl", "dataSourceConfig"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PatchDataSourceConfigStatusResponse:
    ...
    def to_dict(self):
        ...
        if self.data_source_config:
            _dict['dataSourceConfig'] = self.data_source_config.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> PatchDataSourceConfigStatusResponse:
        """Create an instance of PatchDataSourceConfigStatusResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PatchDataSourceConfigStatusResponse.parse_obj(obj)

        _obj = PatchDataSourceConfigStatusResponse.parse_obj({
            "sso_url": obj.get("ssoUrl"),
            "data_source_config": DataSourceConfig.from_dict(obj.get("dataSourceConfig")) if obj.get("dataSourceConfig") is not None else None
        })
        return _obj
