 ...
 * ListScannedFilesOK
 */
@JsonPropertyOrder({ ListScannedFilesOK.JSON_PROPERTY_SCANNED_FILES })
        ...
        if (getScannedFiles() != null) {
            for (int i = 0; i < getScannedFiles().size(); i++) {
                if (getScannedFiles().get(i) != null) {
                    joiner.add(getScannedFiles().get(i).toUrlQueryString(String.format("%sscannedFiles%s%s", prefix,
                            suffix,
