...
/**
 * unsignedCredentialParams. Used to build an unsigned credential before the signing. This param is not accepted when
 * \&quot;unsignedCredential\&quot; is given
 */
@JsonPropertyOrder({ SignCredentialInputDtoUnsignedCredentialParams.JSON_PROPERTY_JSON_LD_CONTEXT_URL,
...
public class SignCredentialInputDtoUnsignedCredentialParams {
    ...
    public SignCredentialInputDtoUnsignedCredentialParams putCredentialSubjectItem(String key,
            Object credentialSubjectItem) {
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(this.jsonLdContextUrl, signCredentialInputDtoUnsignedCredentialParams.jsonLdContextUrl)
                ...
                && Objects.equals(this.credentialSubject,
                        signCredentialInputDtoUnsignedCredentialParams.credentialSubject)
    ...
    public String toUrlQueryString(String prefix) {
        ...
        if (getTypeName() != null) {
            try {
                joiner.add(String.format("%stypeName%s=%s", prefix, suffix,
                        URLEncoder.encode(String.valueOf(getTypeName()), "UTF-8").replaceAll("\\+", "%20")));
            } catch (UnsupportedEncodingException e) {
                // Should never happen, UTF-8 is always supported
                throw new RuntimeException(e);
            }
            ...
            for (String _key : getCredentialSubject().keySet()) {
                try {
                    joiner.add(String.format("%scredentialSubject%s%s=%s", prefix, suffix,
                            ...
                            getCredentialSubject().get(_key),
                            URLEncoder.encode(String.valueOf(getCredentialSubject().get(_key)), "UTF-8")
                                    .replaceAll("\\+", "%20")));
                } catch (UnsupportedEncodingException e) {
                    // Should never happen, UTF-8 is always supported
                    throw new RuntimeException(e);
        ...
        if (getExpiresAt() != null) {
            try {
                joiner.add(String.format("%sexpiresAt%s=%s", prefix, suffix,
                        URLEncoder.encode(String.valueOf(getExpiresAt()), "UTF-8").replaceAll("\\+", "%20")));
            } catch (UnsupportedEncodingException e) {
                // Should never happen, UTF-8 is always supported
                throw new RuntimeException(e);
            }
