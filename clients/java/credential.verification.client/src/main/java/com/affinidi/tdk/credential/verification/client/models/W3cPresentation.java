 ...
 * W3cPresentation
 */
@JsonPropertyOrder({ W3cPresentation.JSON_PROPERTY_AT_CONTEXT, W3cPresentation.JSON_PROPERTY_ID,
        W3cPresentation.JSON_PROPERTY_TYPE, W3cPresentation.JSON_PROPERTY_HOLDER,
        W3cPresentation.JSON_PROPERTY_VERIFIABLE_CREDENTIAL, W3cPresentation.JSON_PROPERTY_PRESENTATION_SUBMISSION,
...
public class W3cPresentation {
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(this.atContext, w3cPresentation.atContext) && equalsNullable(this.id, w3cPresentation.id)
    ...
    @Override
    public int hashCode() {
        return Objects.hash(atContext, hashCodeNullable(id), type, holder, verifiableCredential, presentationSubmission,
                proof);
    }
    ...
    public String toUrlQueryString(String prefix) {
        ...
        if (getHolder() != null) {
            try {
                joiner.add(String.format("%sholder%s=%s", prefix, suffix,
                        URLEncoder.encode(String.valueOf(getHolder()), "UTF-8").replaceAll("\\+", "%20")));
            } catch (UnsupportedEncodingException e) {
                // Should never happen, UTF-8 is always supported
                throw new RuntimeException(e);
            }
        ...
        if (getVerifiableCredential() != null) {
            for (int i = 0; i < getVerifiableCredential().size(); i++) {
                if (getVerifiableCredential().get(i) != null) {
                    joiner.add(getVerifiableCredential().get(i).toUrlQueryString(String.format(
                            "%sverifiableCredential%s%s", prefix, suffix,
                            "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix))));
                }
            }
        }
        ...
        if (getProof() != null) {
            try {
                joiner.add(String.format("%sproof%s=%s", prefix, suffix,
                        URLEncoder.encode(String.valueOf(getProof()), "UTF-8").replaceAll("\\+", "%20")));
            } catch (UnsupportedEncodingException e) {
                // Should never happen, UTF-8 is always supported
                throw new RuntimeException(e);
            }
