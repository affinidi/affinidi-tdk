 ...
 */
@JsonPropertyOrder({ StartIssuanceInputDataInner.JSON_PROPERTY_CREDENTIAL_TYPE_ID,
...
public class StartIssuanceInputDataInner {
    ...
    public StartIssuanceInputDataInner statusListDetails(
    ...
    public StartIssuanceInputDataInner addStatusListDetailsItem(
            StartIssuanceInputDataInnerStatusListDetailsInner statusListDetailsItem) {
    ...
    /**
     * Types of status lists to which the credential should be added once issued. If not provided or empty, the
     * credential is not added to any of the status lists.
     *
            ...
            for (String _key : getCredentialData().keySet()) {
                try {
                    joiner.add(String.format("%scredentialData%s%s=%s", prefix, suffix,
                            ...
                            getCredentialData().get(_key),
                            URLEncoder.encode(String.valueOf(getCredentialData().get(_key)), "UTF-8").replaceAll("\\+",
                                    "%20")));
                } catch (UnsupportedEncodingException e) {
                    // Should never happen, UTF-8 is always supported
                    throw new RuntimeException(e);
        ...
        if (getStatusListDetails() != null) {
            for (int i = 0; i < getStatusListDetails().size(); i++) {
                if (getStatusListDetails().get(i) != null) {
                    joiner.add(getStatusListDetails().get(i).toUrlQueryString(String.format("%sstatusListDetails%s%s",
                            prefix, suffix,
