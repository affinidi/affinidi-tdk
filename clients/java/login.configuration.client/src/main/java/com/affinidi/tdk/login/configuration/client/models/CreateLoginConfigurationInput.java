 ...
 */
@JsonPropertyOrder({ CreateLoginConfigurationInput.JSON_PROPERTY_NAME,
...
public class CreateLoginConfigurationInput {
    ...
    @Override
    public int hashCode() {
        return Objects.hash(name, description, redirectUris, postLogoutRedirectUris, vpDefinition,
                presentationDefinition, idTokenMapping, clientMetadata, claimFormat, failOnMappingConflict, scope,
    ...
    public String toUrlQueryString(String prefix) {
        ...
        if (getClaimFormat() != null) {
            try {
                joiner.add(String.format("%sclaimFormat%s=%s", prefix, suffix,
                        URLEncoder.encode(String.valueOf(getClaimFormat()), "UTF-8").replaceAll("\\+", "%20")));
            } catch (UnsupportedEncodingException e) {
                // Should never happen, UTF-8 is always supported
                throw new RuntimeException(e);
            }
        ...
        if (getFailOnMappingConflict() != null) {
            try {
                joiner.add(String.format("%sfailOnMappingConflict%s=%s", prefix, suffix, URLEncoder
                        .encode(String.valueOf(getFailOnMappingConflict()), "UTF-8").replaceAll("\\+", "%20")));
            } catch (UnsupportedEncodingException e) {
                // Should never happen, UTF-8 is always supported
                throw new RuntimeException(e);
            }
