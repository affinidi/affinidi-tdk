...
- If the request includes an [**`assertion`**](#assertion-validation-by-issuer) to issue a credential different from the sender, the implementer **MUST** cryptographically verify it to prove control of the Holder DID.
...
2. **Feature Disclosure**
    ...
    The Issuer **MAY** respond with a message listing the supported features ([`discover-features/2.0/disclose`](#feature-disclosures)).
...
The `require-issuance` message is optional. The Issuer **MAY** issue a credential without the Holder initiating feature discovery or receiving a request.
...
- **`proposal_id` REQUIRED:** References to a unique identifier for the out-of-band offer generated by the Issuer to identify the specific request when initiated by the Holder (e.g., when the Holder clicks on a CTA button requesting to issue an EmailVC). **REQUIRED** when initiating a request to issue a credential.

- **`credential_format` OPTIONAL:** The format of the Verifiable Credential to be issued (e.g., W3C/V2 Data Model). This field is optional, and the Issuer may choose to issue using one of the supported format from the list.
...
- **`data_integrity_proof_suite` OPTIONAL:** An optional configuration that specify which data integrity proof suite Holder requires. It can be configured only for VC type that supports it. If not provided, the Issuer can choose using one of the proof suite it supports.

**`json_web_signature_algorithm` OPTIONAL:** An optional configuration that specify which JSON Web Signature algorithm Holder requires. It can be configured only for VC type that supports it. If not provided, the Issuer can choose using one of the the supported JSON Web Signature from the list.
...
- **`credential` REQUIRED:** Contains the issued credential to the Holder, the value can be JSON or serialised string, based on credential type.
...
- **`nonce` RQUIRED** a UUID value used only once per switch-context request. The Issuer **MUST** validate the nonce value to prevent reuse of the message.
