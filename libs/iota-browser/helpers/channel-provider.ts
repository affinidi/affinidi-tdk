...
import { PrepareRequestEvent, SignedRequestEventSchema } from '../validators/events'
...
export class ChannelProvider {
    ...
    private async createConfigBuilder(iotaCredentials: IotaCredentials): Promise<iot.AwsIotMqtt5ClientConfigBuilder> {
        ...
        configBuilder.withSessionBehavior(mqtt5.ClientSessionBehavior.RejoinAlways).withConnectProperties({
    ...
    private async startMqttClient(iotaConfigBuilder: iot.AwsIotMqtt5ClientConfigBuilder): Promise<mqtt5.Mqtt5Client> {
            ...
            client.on('messageReceived', (messageReceivedEvent: mqtt5.MessageReceivedEvent): void => {
                if (messageReceivedEvent.message.payload) {
                    ...
                    try {
                        ...
                        if (event.eventType === 'signedRequest' && correlationId === event.correlationId) {
