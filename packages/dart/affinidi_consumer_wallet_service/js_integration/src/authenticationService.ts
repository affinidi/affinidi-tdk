...
class AuthenticationService {
  ...
  public encodeObjectToJWT(jwtObject: { header: AnyObject; payload: AnyObject; signature: Buffer }) {
  ...
  public signJWT(input: { header: AnyObject; payload: AnyObject }, ks: KeysService): string {
    const toSign = [encode(JSON.stringify(input.header)), encode(JSON.stringify(input.payload))].join('.')
  ...
  signAssertion(encryptionSeed: string, seedPassword: string, aud: string) {
  ...
  getSignedAssertionObject(encryptionSeed: string, seedPassword: string, did: string, aud: string) {
